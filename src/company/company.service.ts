...
export class CompanyService {
  ...
  async findAll(query: {
    ...
    const person = (
      await this.userService.findOneByIdWithRelations(query.user.id, ["person"])
    )?.person;
    if (!person)
      throw new NotFoundException("Cadastro de pessoa não encontrado");
    ...
    return {
      ...
      currentPage: page,
      nextPage:
        total / limit > page ? `?page=${page + 1}&limit=${limit}` : null,
  ...
  async createForPerson(companyData: CreateCompanyDto, user: User) {
    ...
    const personCompany = await this.personService.findOneById(
  ...
  async validateCompany(companyData: CreateCompanyDto) {
    ...
    if (
      !companyData.stateInscr ||
      !validateStateInscr(companyData.stateInscr)
    ) {
    ...
    if (companyData.stateInscr) {
      const existingCompanyByStateInscr = await this.findOneByStateInscr(
        companyData.stateInscr
      );
      if (existingCompanyByStateInscr) {
        throw new BadRequestException(
          "Já existe uma empresa com essa Inscrição Estadual"
        );
      }
    }
  }
